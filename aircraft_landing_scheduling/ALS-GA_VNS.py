'''  Aircraft Landing Scheduling (Static Case) - GA-VNS'''
import numpy as np
import os
import random
...
def relocate(x0, num_planes):
    points = np.random.choice(range(0, num_planes), size = 2, replace=False)
    x1 = np.copy(x0)
    x1[points[1]] = x0[points[0]]
    if points[0] > points[1]:
        x1[points[1]+1:points[0]+1] = x0[points[1]:points[0]]
    else:
        x1[points[0]:points[1]] = x0[points[0]+1:points[1]+1]   
    return x1    
        
def opt2(x0, num_planes):
    x1 = np.copy(x0)
    points = np.random.choice(range(0, num_planes), size = 2, replace=False)
...
def opt3(x0, num_planes):
    x1 = np.copy(x0)
    points = np.random.choice(range(0, num_planes), size = 3, replace=False)
    sorted_points = np.sort(points)
    point1 = sorted_points[0]
    point2 = sorted_points[1]
    point3 = sorted_points[2]
    x1[point1:point2+1] = np.fliplr([x0[point1:point2+1]])[0]
    x1[point2+1:point3+1] = np.fliplr([x0[point2+1:point3+1]])[0]
    return x1
...
def vns(x0, population_size, num_planes, plane_details, sep_time):     
    m = 3      #max reps without improvement
    tmax = 2   #better solutions
    rep = 0
    t = 0    
    functions = (relocate, opt2, opt3)
    while (rep < m and t < tmax):
        new_rep = True
        for fnc in functions:         
            x1 = fnc(x0, num_planes)
            x2 = fnc(x1, num_planes)
            ...
            if x2fit > x0fit:
                ...
                if t > 0:
                    break
            if fnc is opt3 and new_rep == True:
                rep+=1                      
    return x0, x0fit, l_time0, invalid0        


def vns_generation(generation, population_size, num_planes, plane_details, sep_time):
    landing_time = np.zeros((population_size, num_planes))
    count_invalid = np.zeros(population_size)
    fitness = np.zeros(population_size)
    for i in range(0, population_size, 1):
        x0 = np.copy(generation[i])
        generation[i], fitness[i], landing_time[i], count_invalid[i] = vns(x0, population_size, num_planes, plane_details, sep_time)     
    max_fitness = np.amax(fitness)
    index = np.where(fitness == max_fitness)
    max_index = index[0][0].astype(int)
    sum_fitness = np.sum(fitness) 
    return generation.astype(int), landing_time, count_invalid, fitness , max_fitness, max_index, sum_fitness    
...
def main():
    ...
    mutation_prob = 0.2
    num_generations = 20
    num_planes,plane_details,sep_time = get_data('airland6.txt')
    ...
    #vns
    first_gen, landing_time, count_invalid, fitness , max_fitness, max_index, sum_fitness = vns_generation(first_gen, population_size, num_planes, plane_details, sep_time)
    #
